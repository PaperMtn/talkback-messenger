...
def test_subscription_initialisation(mock_subscription):
    ...
    assert mock_subscription.name == 'cloud security'
    assert mock_subscription.rank == 20
    assert mock_subscription.resource_types == ['post', 'news']
    assert mock_subscription.users == ['tobias.funke@example.com']
    assert mock_subscription.channels == ['C01234567']
...
def test_subscription_creation_missing_fields():
    ...
    assert mock_subscription.subscription_type == 'category'
    assert mock_subscription.name == 'firefox'
    assert mock_subscription.rank == 80.00
    assert mock_subscription.resource_types == ['post', 'news', 'oss', 'video', 'paper', 'slides', 'n_a']
    assert not mock_subscription.curated
    assert mock_subscription.users is None
    assert mock_subscription.channels is None
...
def test_subscription_creation_invalid_rank():
    ...
    assert subscription_object.rank == 80.00
